...
class AI:
    ...
    def determine_next_move_abs(self, max_depth, turn):
        ...
        for depth in range(1, max_depth + 1):
            ...
            score, move, nodes = self.alpha_beta_search(game.board, turn, depth, -math.inf, math.inf, True, total_start_time)
    ...
    def alpha_beta_search(self, board, player, depth, alpha, beta, maximizing_player, start_time):
        ...
        if maximizing_player:
            ...
            for move in valid_moves:
                new_board = self.simulate_move(board, move)
                eval, _, nodes = self.alpha_beta_search(new_board, switch_player(player), depth - 1, alpha, beta, False, start_time)
                nodes_searched += nodes
                if eval > max_eval:
                    max_eval = eval
                    best_move = move
            ...
            for move in valid_moves:
                new_board = self.simulate_move(board, move)
                eval, _, nodes = self.alpha_beta_search(new_board, switch_player(player), depth - 1, alpha, beta, True, start_time)
                nodes_searched += nodes
                if eval < min_eval:
                    min_eval = eval
                    best_move = move
